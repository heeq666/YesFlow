...
> v0.3.0 · 一个把 AI 规划、任务拆解、资料沉淀与执行推进整合到同一张画布里的可视化工作流工作台。
...
YesFlow 会把一句自然语言目标转成可以继续编辑的执行流程图。你可以先让 AI 生成计划，再在节点画布上继续整理结构，并把每个任务节点需要的文档、链接、时间安排和表格都沉淀在同一个工作空间里。

`0.3.0` 开始，YesFlow 不再只是一个简单的流程编辑器，而更像一个面向复杂任务的可视化 AI 工作台。

## 0.3.0 更新重点

- 新增节点工具工作区，支持 `文档`、`表格`、`链接`、`时间` 四类节点附属内容
- 新增右侧工具栏与日历视图，节点信息和时间规划可以并排查看
- 内置多模型配置，支持 MiniMax、DeepSeek、通义千问、豆包、智谱以及 OpenAI Compatible 接口
- 新增项目记录区，支持本地持久化、快速载入和拖拽排序
- 流程编辑进一步增强，支持编组、连线标签与颜色、导入导出、外观预设和更多交互配置
